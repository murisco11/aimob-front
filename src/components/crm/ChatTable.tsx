...
const formatShortTime = (dateInput: Date | string) => {
  const now = new Date();
      ...
      <div className="bg-card rounded-xl border border-border overflow-hidden w-full h-full flex flex-col">
        ...
        <div className="divide-y divide-border overflow-y-auto flex-1 scrollbar-thin">
          {displayedChats.map((chat) => (
              <div
                  key={chat.id}
                  className={`w-full flex items-center  px-3 py-3.5 transition-colors hover:bg-muted/50 group ${selectedChatId === chat.id ? "bg-primary/5 border-l-2 border-l-primary" : "border-l-2 border-l-transparent"
                  }`}
              >
                <button
                ...
                >
                  ...
                  <span className="text-sm font-medium text-card-foreground truncate">
                    ...
                    {chat.lastMessageAt && formatShortTime(chat.lastMessageAt)}
                  </span>
                    </div>

                    <div className="flex items-center mt-0.5">
                      <p className="text-[11px] text-muted-foreground truncate italic w-full">
                        {chat.lastMessage}
                      </p>
                    </div>
                  </div>
