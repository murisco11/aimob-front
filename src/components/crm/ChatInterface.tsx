...
const ChatInterface = ({chat}: ChatInterfaceProps) => {
    ...
    return (
        <div className="w-full flex flex-col h-full bg-card rounded-xl border border-border overflow-hidden">
            <div className="px-5 py-3.5 border-b border-border flex items-center justify-between z-10 bg-card">
                <div className="flex items-center gap-3">
                    <div className="relative">
                        <div
                            className="w-9 h-9 rounded-full bg-secondary flex items-center justify-center text-xs font-semibold text-secondary-foreground">
                            {avatarLetter}
                        </div>
                        <div
                            className={`absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 border-card ${chat.status === 'open' ? 'bg-success' : 'bg-muted-foreground'}`}/>
                    </div>
                    <div>
            ...
            {chat.lead.description && (
                <div className="mx-4 mt-3 p-3.5 bg-primary/5 border border-primary/15 rounded-lg shrink-0">
                    <div className="flex items-center gap-2 mb-2">
                        <span className="text-xs font-semibold text-primary">Descrição do lead</span>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <p className="text-xs font-medium text-card-foreground mt-0.5 capitalize">{chat.lead.description}</p>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 scrollbar-thin">
                {isLoading ? (
                    <div className="flex items-center justify-center h-full text-sm text-muted-foreground">Carregando
                        mensagens...</div>
                ) : mensagens.length === 0 ? (
                    <div className="flex items-center justify-center h-full text-sm text-muted-foreground">Nenhuma
                        mensagem encontrada.</div>
                ) : (
                    <>
                        {mensagens.map((msg) => {
                            ...
                            return (
                                <div key={msg.id}
                                     className={`flex w-full group ${!msg.fromMe ? "justify-start" : "justify-end"}`}>

                                    <div
                                        className={`flex items-center gap-2 max-w-[85%] ${msg.fromMe ? "flex-row-reverse" : "flex-row"}`}>
                                        ...
                                        >
                                            {msg.quotedMessageBody && (
                                                <div
                                                    className={`mb-2 p-2 rounded-lg text-xs border-l-4 ${!msg.fromMe ? "bg-background/40 border-primary" : "bg-black/10 border-black/30"}`}>
                                                    <span
                                                        className="font-semibold block mb-0.5">Mensagem respondida:</span>
                            ...
                            {msg.quotedMessageBody}
                                            ...
                                            {msg.fromMe && msg.type === "ai" && (
                                                <div className="flex items-center gap-1 mb-1">
                                                    ...
                                                    <span
                                                        className="text-[10px] font-medium text-primary">AI Auto-reply</span>
                                                </div>
                                            )}
            ...
            <div className="p-3 border-t border-border flex flex-col gap-2 bg-card">
                ...
                {replyingTo && (
                    <div
                        className="flex items-center justify-between bg-muted p-2 rounded-lg border-l-4 border-primary text-sm mx-1">
                        <div className="flex flex-col truncate">
                            <span
                                className="font-semibold text-xs text-primary">{replyingTo.fromMe ? "Você" : leadName}</span>
